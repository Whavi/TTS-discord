...
class infinityDeconnexion(commands.Cog):
    # Constructeur de la classe muteDefean
    def __init__(self, bot):
        self.bot = bot

    @app_commands.command(name="deconnexion_infinity", description="décoonecte de façon permanente pour la personne choisie sans possibilité de retour en arrière")
    # Seulement les administrateurs peuvent utiliser cette commande
    @app_commands.default_permissions(administrator=True)
    async def infinityDeconnexion_slash(self, interaction: discord.Interaction, membre: discord.Member):
        ...
        if not voice_client_membre:
            ...
            while True:
                await membre.edit(voice_channel=None) # déconnecter l'utilisateur à l'infini                
    ...
    @infinityDeconnexion_slash.error
...
async def setup(bot):
    await bot.add_cog(infinityDeconnexion(bot))
